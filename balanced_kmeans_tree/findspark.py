...
def read_extra_variables(filePath):
    lines = []
    try:
        f = open(filePath,"r")
        lines = f.readlines()
    ...
    for line in lines:
        if len(line.strip()) == 0:
            continue
        kv = line.split('=')
        env_name = kv[0].split()[1].strip()
        env_value = kv[1].strip()
        os.environ[env_name] = env_value


def init(spark_home=None, extra_env_file=None, python_path=None, edit_rc=False, edit_profile=False):
    ...
    if extra_env_file:
        read_extra_variables(extra_env_file)
...
#MLP-71 begin
def add_python_zips(zips):

    """Add extra python zip to the pyspark interpreter.

    Set the PYSPARK_SUBMIT_ARGS properly.

    Parameters
    ----------
    zips: list of path to zips in string format
    """
    ...
    _add_to_submit_args("--py-files "+ ",".join(zips))

def set_executor_mem(size):

    _add_to_submit_args("--executor-memory " + size)

def set_driver_mem(size):

    _add_to_submit_args("--driver-memory " + size)

#MLP-96 begin
def set_driver_class_path(jars):

    #if the parameter is a string, convert to a single element list
    if isinstance(jars, str):
        jars = [jars]

    _add_to_submit_args("--driver-class-path "+ ":".join(jars))
#MLP-96 end

#MLP-81 begin
def set_app_name(name):

    _add_to_submit_args("--name " + name)
#MLP-81 end

def end():

    _add_to_submit_args("pyspark-shell")
#MLP-71 end
